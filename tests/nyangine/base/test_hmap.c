...
s32 main(void) {
  ...
  for (u32 i = 0; i < 20; ++i) {
    nya_hmap_set(&collision_map, i, (u64)(i * 100));
  }
  ...
  for (u32 i = 0; i < 10; ++i) {
    nya_hmap_remove(&collision_map, i);
  }
  ...
  for (u32 i = 0; i < 10; ++i) {
    nya_hmap_set(&remove_all_map, i, (u64)i);
  }
  for (u32 i = 0; i < 10; ++i) {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    NYA_RNGDistribution val_dist = {.type = NYA_RNG_DISTRIBUTION_UNIFORM, .uniform = {.min = 0, .max = 10000}};
