...
#define _NYA_ASSERT1(condition)              if (!(condition)) { nya_panic("Assertion failed: %s", #condition); }
#define _NYA_ASSERT2(condition, message)     if (!(condition)) { nya_panic("Assertion failed: %s, %s", #condition, message); }
#define _NYA_ASSERT3(condition, format, ...) if (!(condition)) { nya_panic("Assertion failed: %s, " format, #condition, __VA_ARGS__); }
#endif // NYA_NO_ASSERT
