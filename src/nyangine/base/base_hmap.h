...
#define nya_hmap_copy(hmap_ptr)                                                                                        \
  ({                                                                                                                   \
    typeof(*(hmap_ptr)) copy = {                                                                                       \
        ...
        .values =                                                                                                      \
            nya_arena_copy((hmap_ptr)->arena, (hmap_ptr)->values, sizeof(*(hmap_ptr)->values) * (hmap_ptr)->capacity), \
...
#define nya_hmap_move(hmap_ptr, new_arena_ptr)                                                                         \
  ({                                                                                                                   \
    ...
    *(hmap_ptr) = (typeof(*(hmap_ptr))){.keys = nya_arena_move(                                                        \
                                            (hmap_ptr)->arena,                                                         \
                                            new_arena_ptr,                                                             \
                                            (hmap_ptr)->keys,                                                          \
                                            sizeof(*(hmap_ptr)->keys) * (hmap_ptr)->capacity                           \
                                        ),                                                                             \
                                        .values = nya_arena_move(                                                      \
                                            (hmap_ptr)->arena,                                                         \
                                            new_arena_ptr,                                                             \
                                            (hmap_ptr)->values,                                                        \
                                            sizeof(*(hmap_ptr)->values) * (hmap_ptr)->capacity                         \
                                        ),                                                                             \
...
#define nya_hmap_foreach_key(hmap_ptr, key_name)                                                                       \
  for (u64 _nya_hmap_foreach_index = 0; _nya_hmap_foreach_index < (hmap_ptr)->capacity; _nya_hmap_foreach_index++)     \
    if ((hmap_ptr)->occupied[_nya_hmap_foreach_index])                                                                 \
      for (typeof((hmap_ptr)->keys[0]) key_name = (hmap_ptr)->keys[_nya_hmap_foreach_index];                           \
...
#define nya_hmap_foreach_value(hmap_ptr, value_name)                                                                   \
  for (u64 _nya_hmap_foreach_index = 0; _nya_hmap_foreach_index < (hmap_ptr)->capacity; _nya_hmap_foreach_index++)     \
    if ((hmap_ptr)->occupied[_nya_hmap_foreach_index])                                                                 \
      for (typeof((hmap_ptr)->values[0]) value_name = (hmap_ptr)->values[_nya_hmap_foreach_index];                     \
           _nya_hmap_foreach_index < (hmap_ptr)->capacity;                                                             \
           _nya_hmap_foreach_index = (hmap_ptr)->capacity)
